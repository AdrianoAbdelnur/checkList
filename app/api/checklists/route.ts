...
export async function POST(req: Request) {
  ...
  if (!templateId) return Response.json({ ok: false, message: "templateId requerido" }, { status: 400 });

  let templateVersion = body?.templateVersion ? Number(body.templateVersion) : null;

  if (!templateVersion) {
    const last = await ChecklistTemplate.findOne({ templateId, isActive: true }).sort({ version: -1 }).lean();
    if (!last) return Response.json({ ok: false, message: "Template no encontrado" }, { status: 404 });
    templateVersion = last.version;
  } else {
    const exists = await ChecklistTemplate.findOne({ templateId, version: templateVersion }).lean();
    if (!exists) return Response.json({ ok: false, message: "Template/version no existe" }, { status: 404 });
  }
